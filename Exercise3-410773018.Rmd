...
```{r ans13}
aa <- classData$`公-10-06` == "有看公告"
bb <- classData$`公-10-06` == "有看公告" & 
...
cc <- classData$`公-10-06` == "沒看公告"
dd <- classData$`公-10-06` == "沒看公告" & 
  classData$`練2018-10-17-1` == "1"
C <- sum(cc)
D <- sum(dd)
D/C -> "佔沒看公告的比例"
...
with_tz(hwData$Timestamp,tzone="Asia/Taipei") -> hwData$Timestamp
```
...
```{r ans23}
with_tz(hwData$Timestamp,tzone = "UK/London")-> hwData$Timestamp
difftime(deadline, hwData$Timestamp, tz, units = "days") -> hwData$early
```
...
```{r ans24}
hwData$early <- as.numeric(hwData$early)
